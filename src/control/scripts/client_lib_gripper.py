...
class kuka_iiwa_ros_client:
    ...
    def __init__(self): # Makes kuka_iiwa ROS node
        ...
        rospy.Subscriber("GripperState", String, self.Gripper_callback)
        rospy.Subscriber("ReadyState", String, self.Ready_callback)
    ...
    def Gripper_callback(self, data):
        d = str(data.data).split() # e.g. off 1459253274.11
        if d[0] == 'True': self.Gripper = True
        if d[0] == 'False': self.Gripper = False
        
    def Ready_callback(self, data):
        d = str(data.data).split() # e.g. off 1459253274.11
        if d[0] == 'True': self.Ready = True
        if d[0] == 'False': self.Ready = False
