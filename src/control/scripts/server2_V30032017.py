...
class iiwa_socket:
    ...
    def socket(self, ip, port):
        ...
        #os.system('clear')
        print('\n==========================================')
        print('<   <  < << SHEFFIELD ROBOTICS >> >  >   >')
        print('==========================================')
        print(' KUKA API for ROS')
        print(' Server Version: ' + version)
        print('==========================================\n')


        print('Starting up on:', 'IP:', ip, 'Port:', port)
        try:
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            sock.bind(server_address)
        except:
            print('Error: ' + "Connection for KUKA cannot assign requested address:", ip, port)
            os._exit(-1)
        ...
        while self.isconnected:
            try:
                ...
                datab = self.connection.recv(self.BUFFER_SIZE)
                ...
                for pack in data.split(">"):  # parsing data pack
                    ...
                    cmd_splt = pack.split()
                    if len(pack) and cmd_splt[0]=='GripperState':  # If it's GripperState
                        if cmd_splt[1] == "false": self.Gripper = (False, last_read_time)
                        elif cmd_splt[1] == "true": self.Gripper = (True, last_read_time)
                        
                    if len(pack) and cmd_splt[0]=='ReadyState':  # If it's ReadyState
                        if cmd_splt[1] == "false": self.Ready = (False, last_read_time)
                        elif cmd_splt[1] == "true": self.Ready = (True, last_read_time)
                ...
                if elapsed_time > 5.0:  # Didn't receive a pack in 5s
                    ...
                    self.isconnected = False
                    print('No packet received from iiwa for 5s!')
...
class kuka_iiwa_ros_node:
    ...
    def __init__(self, ip, port): # Makes kuka_iiwa ROS node
        ...
        pub_GripperState = rospy.Publisher('GripperState', String, queue_size=10)
        pub_ReadyState = rospy.Publisher('ReadyState', String, queue_size=10)
        #   Make kuka_iiwa node
        ...
        while not rospy.is_shutdown() and self.iiwa_soc.isconnected:
            ...
            for [pub, data] in [ [pub_JointPosition, self.iiwa_soc.JointPosition],
                                 ...
                                 [pub_GripperState, self.iiwa_soc.Gripper],
                                 [pub_ReadyState, self.iiwa_soc.Ready]]:
...
def read_conf():
    ...
    if os.path.isfile(f_conf):
        ...
        if len(IP.split('.'))!=4 or Port<=0:
            print('Error:', "conf.txt doesn't include correct IP/Port! e.g. server 172.31.1.50 port 1234")
            exit()
    else:
        print('Error:', "conf.txt doesn't exist!")
        exit()
